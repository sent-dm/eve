...
Completed Workflow steps replay their recorded results. A retried turn step also reuses its committed checkpoint without repeating the work. If a turn attempt started effects but failed before committing its resulting checkpoint, eve fails the turn rather than repeating work whose outcome is uncertain. This does not guarantee exactly-once external effects: keep operations such as charges or emails idempotent. Events already written stay on the session stream, and model retries within an executing step can emit additional events — see [the event envelope](./sessions-runs-and-streaming#the-event-envelope).
...
Channels authorize the input, resolve its session address, and start an independent turn workflow. After claiming execution ownership, the winning turn resolves the immutable resource descriptor and restores conversation history inside its execution step. The session event stream stays stable across turns and deployments.
