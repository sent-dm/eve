...
Set `privateToolApprovals` when tool inputs must be visible only to one reviewer. eve sends the preview and controls to the reviewer's direct messages with a link to the session thread, routes the response back to the original session, and posts `Waiting on approval from @reviewer…` in that thread without exposing the tool input. Add the `im:write` bot scope. By default eve targets the Slack user who triggered the turn, or provide `reviewer` to resolve another user:
