...
`ctx.abortSignal` aborts when the run is cancelled: an interrupted turn for a waiting tool, `task_cancel`
...
After the signal fires, cancellation completes when the body and its awaited work finish
unwinding. `ask()` observes the signal; raw hooks and Workflow `sleep` do not. Pass the signal to
interruptible steps and arrange for other waits to settle. A body that ignores cancellation keeps
the run pending until it settles.
...
updates the active turn at its next boundary. An `interrupt` message cancels the turn and the
tool run, withdrawing its requests. Input responses never steer.
